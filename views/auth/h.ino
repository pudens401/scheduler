...
// ---------- WiFi Config ----------
const char* SSID_AP = "Ringer";
const char* PASS_AP = "12345678";
String deviceId = "RNG01";
...
// ---------- API ----------
const char* API_URL = "https://smart-scheduler-s5q7.onrender.com/schedule/RNG01";
...
// Topics
const char* TOPIC_RING     = "GD/RNG/V2/RING";
const char* TOPIC_SILENT   = "GD/RNG/V2/SILENT";
const char* TOPIC_SCHEDULE = "GD/RNG/V2/SCHEDULE";
...
bool fetchScheduleFromAPI() {
  ...
  if (doc.containsKey("times")) {
    ...
    for (JsonVariant t : timesArray) {
      String timeStr;
      // handle both {"time":"HH:MM"} or simple "HH:MM"
...
void updateRelay() {
  if (relayActive && millis() - relayOnTime >= 5000) {
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (t == TOPIC_RING) {
    ...
    if (!fetchScheduleFromAPI()) {
      Serial.println("Failed to fetch schedule from API via MQTT trigger");
    }
  }
}
...
void setup() {
  ...
  // Initialize MQTT client settings regardless of WiFi state.
...
void loop() {
  if (WiFi.getMode() == WIFI_AP) {
    ...
    // call loop even if not connected (PubSubClient requires it)
    mqttClient.loop();
  }

  updateRelay();

  // Silent button handling (with debounce)
  ...
  // Ring button handling (momentary trigger, override silent)
